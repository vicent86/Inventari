        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ url('pagos/'.$pago->id.'/editar') }}" method="POST">
                            @csrf
                            @method("PUT")

                            <div class="mb-3">
                                <label>ID_Venta</label>
                                <input type="text" class="from-control" name="id_venta" value="{{ $pago->id_venta }}" />
                                @error('id_venta') <span class="text-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="mb-3">
                                <label for="importe">Importe</label>
                                <input type="text" class="from-control" name="importe" value="{{ $pago->importe }}" />
                                @error('importe') <span class="text-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="mb-3">
                                <label for="iva">IVA</label>
                                <input type="text" class="from-control" name="iva" value="{{ $pago->iva }}" />
                                @error('iva') <span class="text-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="mb-3">
                                <label for="total">Total</label>
                                <input type="text" class="from-control" name="total" value="{{ $pago->total }}" />
                                @error('total') <span class="text-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="mb-3">
                                <label>Fecha_Pago</label>
                                <input type="date" class="from-control" name="fecha_pago" value="{{ $pago->fecha_pago }}" />
                                @error('fecha_pago') <span class="text-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="mb-3">
                                <label>Metodo_Pago</label>
                                <input type="checkbox" name="metodo_pago" {{ $pago->metodo_pago == true ? 'checked':'' }} />
